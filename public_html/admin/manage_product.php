...
foreach ($attrProduct as $index => $attr) {
    echo '<input type="hidden" name="attr_product_id[]" value="' . $attr['product_id'] . '">';
}
...
mysqli_query($con,"insert into product(categories_id,name,short_desc,description,meta_title,meta_desc,meta_keyword,status,image,best_seller,sub_categories_id,added_by) values('$categories_id','$name','$short_desc','$description','$meta_title','$meta_desc','$meta_keyword',1,'$image','$best_seller','$sub_categories_id','".$_SESSION['ADMIN_ID']."')");
	...
	if(isset($_POST['mrp'])){
			foreach($_POST['mrp'] as $key=>$val){
				...
				if($attr_id>0){
					mysqli_query($con,"update product_attributes set size_id='$size_id',color_id='1',mrp='$mrp',price='$price',qty='$qty' where id='$attr_id'");
				}else{
					mysqli_query($con,"insert into product_attributes(product_id,size_id,color_id,mrp,price,qty) values('$id','$size_id','1','$mrp','$price','$qty')");
				}
			}
		}
								...
								<div class="form-group"  id="product_attr_box">
									...
									<div class="row"id="attr_<?php echo $attrProductLoop?>">
									  ...
									  <div class="col-lg-2">
										...
										<option>Size</option>
										<?php
											$res=mysqli_query($con,"select id,size from size_master order by id asc");
											while($row=mysqli_fetch_assoc($res)){
