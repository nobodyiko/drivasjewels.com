...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$res=mysqli_query($con,"select * from custom where id='$id'");
	...
	if($check>0){
		...
		header('location:mans_custom.php');
		die();
	}
}
...
if(isset($_POST['submit'])){
	...
	$res=mysqli_query($con,"select * from custom where name='$name'");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
...
move_uploaded_file($_FILES['image']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . "/media/product/" . $image);
				$update_sql="update custom set name='$name',link='$link',image='$image' where id='$id'";
			}else{
				$update_sql="update custom set name='$name',link='$link' where id='$id'";
			}
...
mysqli_query($con, "INSERT INTO custom(name, link, image) VALUES ('$name', '$link', '$imagePath')");
...
<div class="content pb-0">
            <div class="animated fadeIn">
               <div class="row">
                  <div class="col-lg-12">
                     <div class="card">
                        <div class="card-header"><strong>Men's Custom Product</strong><small> Form</small></div>
                        <form method="post" enctype="multipart/form-data">
							<div class="card-body card-block">
							   
								
								<div class="form-group">
									<label for="categories" class=" form-control-label">Catogiry Name</label>
