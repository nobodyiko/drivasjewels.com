...
<div class="content pb-0">
					  ...
					  <table class="table">
								<thead>
									<tr>
										...
										<th class="product-thumbnail">Product Attributes</th>
								...
								<tbody>
									<?php
							$res = mysqli_query($con, "SELECT DISTINCT(order_detail.id), order_detail.*, product.name, product.image, `order`.address, `order`.city, `order`.pincode, product_attributes.size_id, size_master.size
    FROM order_detail
    INNER JOIN product ON order_detail.product_id = product.id
    INNER JOIN `order` ON order_detail.order_id = '$order_id'
    INNER JOIN product_attributes ON order_detail.product_attr_id = product_attributes.id
    INNER JOIN size_master ON product_attributes.size_id = size_master.id
    ...
    GROUP BY order_detail.id");
						...
						<div id="address_details">
							...
							<div>
								<form method="post">
									<select class="form-control" name="update_order_status" required style="border: 0.5px solid #d9e0e7;
    margin-top: 10px;
    margin-bottom: 10px;">
