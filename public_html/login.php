...
<head>
    ...
    <title>Driva's Jewels || Login Now</title>
...
<div class="g-recaptcha" data-sitekey="6Le2nGIoAAAAAJNPhS7VUGl5Go5_GbD8pWSYnAiw">
    
</div>
							    		<div class="form-footer">
							    			<button type="button" onclick="user_login()" class="btn btn-outline-primary-2">
			                					<span>LOG IN</span>
                                    ...
                                    <p class="text-center">Don't Have a Account ? <a href="register">Register Now</a></p>
									<br>
    ...
    <?php
        if(isset($_POST['g-recaptcha-response'])){
            ...
            if(!$recaptcha){
                echo'<script>alert()"Please Check Re Captcha Box</script>';
            }
        }
    ?>
    <script>
	...
	if(email==""){
		...
		jQuery("#login_email").addClass("error-border"); // Add the error-border class
        is_error = 'yes';
	}if(password==""){
		jQuery('#login_password_error').html('Please enter password');
		 jQuery("#login_password").addClass("error-border"); // Add the error-border class
        is_error = 'yes';
	}
	if(is_error==''){
		...
		jQuery.ajax({
			...
			success:function(result){
				result=result.trim();
				if(result=='wrong'){
					jQuery('.login_msg p').html('Please enter correct password');
					jQuery("#login_email, #login_password").removeClass("error-border");
				}
				if(result=='valid'){
					window.location.href='index';
				}
				if(result=='not_exist'){
				jQuery('.login_msg p').html('Your Account Does Not Exist Please Register us');
				}
			}	
		});
	}	
